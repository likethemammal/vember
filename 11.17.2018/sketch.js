...
const sketch = ({ context, canvas }) => {
  ...
  function createStar(_, i) {
    ...
    const cylinderWidth = 0.75
    const torusRadius = 0.05
    const torusWidth = cylinderWidth + torusRadius
    const cylinderHeight = 0.5

    const cylinderPosition = 0.7
    const sphereWidth = 0.08
    ...
    const segments = 100
    const cylinderSegments = 100

    const box = new THREE.Mesh(
      new THREE.CylinderGeometry(cylinderWidth, cylinderWidth, cylinderHeight, cylinderSegments ),
      finalMaterial,
    )
    ...
    const torus = new THREE.Mesh(
      new THREE.TorusGeometry(torusWidth, torusRadius, segments, segments),
      finalMaterial,
    )
    ...
    const cylinderCenter = -(cylinderHeight / 2) - cylinderPosition

    torus.rotation.x = Math.PI / 2
    torus1.rotation.x = Math.PI / 2

    torus.position.y = -cylinderPosition
    torus1.position.y = -(cylinderHeight + cylinderPosition)
    box.position.y = cylinderCenter


    const numSphere = 15
    const group = new THREE.Group()

    const spheres = [...Array(numSphere)].map((_, i) => {
      const angle = calcAngleFromCenter(i, numSphere)

      const xOrigin = calcXOnCircle(0, cylinderWidth, angle)
      const yOrigin = calcYOnCircle(0, cylinderWidth, angle)

      const sphere = new THREE.Mesh(
        new THREE.SphereGeometry(sphereWidth, 32, 32),
      ...
      sphere.position.x = xOrigin
      sphere.position.z = yOrigin

      sphere.position.y = cylinderCenter
  ...
  const ambientLight2 = new THREE.AmbientLight('#ff781b', 0.2)

  const light1 = new THREE.PointLight('#fff063', 0.1, 100 )
  ...
  return {
    ...
    render ({ time, playhead }) {
      ...
      crown.rotation.y = playhead * Math.PI * 2
